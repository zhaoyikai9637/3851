...
## 13. 2026-09-11 历史日期筛选禁止未来日期

- 按通知中心和邮件发送历史的语义，将 From/To 限制为 UTC+08 当天或更早；当天仍可查询。
- 前端原生日历设置动态最大日期并禁用未来选择，保留英区 `MM/DD/YYYY` 显示；键盘输入未来日期时显示明确英文错误，不发请求。
- 后端通知与日志查询使用同一 UTC+08 规则，直接调用 API 传未来日期返回 422，不能绕过页面限制。
- OpenAPI 已说明历史日期范围；新增前端、验证器和 Supertest 正反向覆盖。最终回归结果记录在 docs/VERIFICATION.md。

## 14. 2026-09-11 日期筛选对齐与滚轮保护

- 将筛选栏帮助文字脱离网格高度计算，保留 `Today or earlier`，同时让类型、From、To 输入框和操作按钮重新处于同一水平线。
- 日期文本框及日历触发控件收到滚轮事件时阻止默认行为并移除焦点，避免滚轮意外改变日期；点击、键盘输入和日历选择保持可用。

## 15. 2026-09-12 HR 工作台整体视觉优化

- 按 `design-taste-frontend` 的审计、排版、间距、色彩与可访问性原则优化现有产品 UI；未套用不适合密集后台的营销页 Hero/动效。
- 保留 JRS 深蓝/钴蓝品牌和全部页面结构、导航、表单、日期校验及业务逻辑；统一语义色、文字层级、控件和卡片风格。
- 通知改为更易扫读的分隔列表，并保留未读状态标识；模板、日志、资料和演示入口采用一致的视觉语言。
- 390px 手机端修复日期帮助文字与筛选按钮相挤的问题；详细设计目的见 `docs/DESIGN_REFRESH.md`，验证见 `docs/VERIFICATION.md`。

## 16. 2026-09-13 新增 Jest 单元测试和自动报告

- 用户明确指定 Jest；保留原有 Vitest 套件，新增 `npm.cmd run test:jest`，只运行独立的后端 Jest 用例，不改变现有 `npm.cmd test`。
