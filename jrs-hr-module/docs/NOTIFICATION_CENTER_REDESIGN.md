...
- A successful bulk update offers an eight-second \`Undo\` action. Undo restores only the notifications changed by that bulk operation.
...
- Undo is intentionally time-limited in the interface. The restore endpoint accepts at most 5,000 IDs to stay within the existing 100 KB JSON request limit.
