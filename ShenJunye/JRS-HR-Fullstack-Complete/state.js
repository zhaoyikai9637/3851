...
(function(root){
  ...
  function apply(db,action,p,context={}){
    ...
    const transition=(to,allowed,summary,notes='')=>{if(!allowed.includes(a.stage))throw Error('This action is not available from '+a.stage+'.');const from=a.stage;a.stage=to;if(notes)a.notes.push({text:notes,at:iso(),actor});return activity(summary,notes,from,to);};
    let result={};
    switch(action){
      case 'move-interview':{
