...
默认数据库名 `jrs_hr_fullstack`；MySQL 8.0.16+，全部表使用 InnoDB，数据库字符集 utf8mb4。`database/schema.sql` 是建表源文件。
...
| hr_application | 候选人与职位的关系、招聘阶段、优先级、拒绝原因 |
...
`db:init` 创建缺失的数据库和表、初始 HR 账号及 5 类模板，不删除记录，也不重置已有密码。`db:seed` 只允许对没有职位、候选人和申请的数据库执行，插入 115 条虚构申请、10 个职位和 24 个历史面试。示例邮箱均为 example.com，发送接口会拒绝向其发送邮件。

`schema.sql` 是首次安装的 v1 结构。之后修改已有表时需增加显式迁移，不能把 `CREATE TABLE IF NOT EXISTS` 当作升级旧表的工具。
